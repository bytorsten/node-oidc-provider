...
module.exports = function getSchema(provider) {
  ...
  const dynamicScopes = [...configuration.dynamicScopes];
  const dynamicScopesLabels = new Set(
    dynamicScopes.map((s) => s[DYNAMIC_SCOPE_LABEL]).filter(Boolean),
  );
    ...
    scopes() {
      if (this.scope) {
        ...
        parsed.forEach((scope) => {
          if (!scopes.has(scope) && !dynamicScopesLabels.has(scope)) {
            if (!dynamicScopes.find((dynamicScope) => dynamicScope.test(scope))) {
